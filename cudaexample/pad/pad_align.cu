    ...
    half* __restrict__ output)
{
    ...
    // 检查填充标志
    bool is_pad = false;
    int in_h = H - pad_h_beg;  // 计算 H 维度的偏移
    int in_w = W - pad_w_beg;  // 计算 W 维度的偏移
    if (in_h < 0 || in_h >= in_H || in_w < 0 || in_w >= in_W) {
        is_pad = true;  // 如果超出边界，标记为填充
    }

    // 遍历每个输出通道（C 维度）
    for (int C = 0; C < out_C; ++C) {
        int in_c = C - pad_c_beg; // 计算 C 维度的偏移
        if (in_c < 0 || in_c >= in_C) {
            is_pad = true;  // 如果 C 超出输入范围，则为填充区域
        }

        int out_offset = ((N * out_H + H) * out_W + W) * out_C + C;

        if (is_pad) {
            output[out_offset] = constant_val;  // 填充值
        } else {
            int in_offset = ((N * in_H + in_h) * in_W + in_w) * in_C + in_c;
            output[out_offset] = input[in_offset];  // 复制输入数据
        }
    }
}
...
int main() {
    // 定义输入张量维度和填充值
    int N = 1, H = 3, W = 2, C = 5;  // 输入大小 (N, H, W, C)
    int pad_n_beg = 0, pad_h_beg = 0, pad_w_beg = 0, pad_c_beg = 0;
    int pad_n_end = 0, pad_h_end = 2, pad_w_end = 0, pad_c_end = 0;  // H 末尾填充 2
    ...
    for (int i = 0; i < N * H * W * C; ++i) {
        h_input[i] = __float2half(i);  
    }
    cudaMemcpy(d_input, h_input, N * H * W * C * sizeof(half), cudaMemcpyHostToDevice);

    // 启动内核
    half constant_val = __float2half(0.0f);  // 填充值为 0.0f

    int block_size = 256;
    int grid_size = (num_elems + block_size - 1) / block_size;

    pad_nhwc8_fp16_constant_kernel<<<grid_size, block_size>>>(num_elems, N, H, W, C,
        out_H, out_W, out_C, pad_n_beg, pad_h_beg, pad_w_beg, pad_c_beg,
        constant_val, (const half*)d_input, (half*)d_output);
